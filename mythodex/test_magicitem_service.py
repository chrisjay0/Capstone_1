...
from magic_items.services import MagicItemService, CollectionService, ItemCollectionService
...
class ItemServiceTestCase(TestCase):
    ...
    def test_collection_service_add_item(self):
        CollectionService.add_magic_item(
            self.u.created_items[0].id,
            self.u.collections[0].id)
        inventory = ItemCollectionModel.query.filter_by(item_id=self.u.created_items[0].id,).first().inventory
    ...
    def test_collection_service_remove_item(self):
        CollectionService.remove_magic_item(
            self.u.created_items[0].id,
            self.u.collections[0].id)
        inventory = ItemCollectionModel.query.filter_by(item_id=self.u.created_items[0].id,).first()
